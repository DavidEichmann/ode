...
drawFrameIx c offset' MotionDataVars{_bs=bs,_rb=rb,_xb=xb,_xsb=xsb,_xeb=xeb} fi = mapM_ drawBone' bs where
     drawBone' :: BoneIx -> IO ()
     drawBone' bi = do
        drawBoneC c (offset' + xsb fi bi) (offset' + xeb fi bi) boneRadiusDisplay
        if showOrient
            then do
                drawBoneC c (offset' + xb fi bi) (offset' + ((xb fi bi) + ((rb fi bi) `rotate` (V3 0.1 0 0)))) (boneRadiusDisplay/3)
                drawBoneC c (offset' + xb fi bi) (offset' + ((xb fi bi) + ((rb fi bi) `rotate` (V3 0 0.1 0)))) (boneRadiusDisplay/3)
            else
                return ()
...
drawFrame offset' j = treeMapM_ drawBone' (set ((view j){offset = (offset $- j) + offset'}) j) where
     ...
     drawBone' j
            | hasParent j   = do
                -- Draw bone
                drawBoneC (BlueA 0.5) (getPosStart j) (getPosEnd j) boneRadiusDisplay
                ...
                drawBoneC (BlueA 0.5) orientStart orientDirection (boneRadiusDisplay/3)
            | otherwise     = return ()
